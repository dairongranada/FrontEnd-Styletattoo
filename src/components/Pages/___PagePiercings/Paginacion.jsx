...
export const Paginacion = ({ pagina, setPagina, maximo }) => {
    ...
    return (
        <div className='containerPrimaryPaginacion'>
            <button className='Buttonpaginator' disabled={pagina === 1 || pagina < 1} onClick={previousPage}>  <svg viewBox="0 0 20 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <p>de</p>
            <p>{maximo}</p>
            <button className='Buttonpaginator' disabled={pagina === Math.ceil(maximo) || pagina > Math.ceil(maximo)}
                onClick={nextPage}>   <svg
                    width="20"
                    height="27"
                    viewBox="0 0 20 27"
                    fill="none"
